...
def test_v13_invalidates_deferred_locators_without_a_parser_policy(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    migrations = database_module._load_migrations()
    legacy_migrations = tuple(item for item in migrations if item[0] <= 12)
    ...
    with database.transaction() as connection:
        connection.execute(
            ...
            ) values (?, 'codex', ?, ?, 'codex-v3', 1, 2, 100, ?,
                      'project_not_found', 'pending', ?, ?, 1,
    ...
    with database.connect(readonly=True) as connection:
        columns = tuple(
            row["name"] for row in connection.execute("pragma table_info(codex_deferred_records)")
        )
        ...
        catchup = connection.execute(
            "select value_json from app_state where name = 'codex_reconcile_catchup_required'"
        ).fetchone()
        quick_check = connection.execute("pragma quick_check").fetchone()[0]
        foreign_key_violations = connection.execute("pragma foreign_key_check").fetchall()

    assert "parser_policy_sha256" in columns
...
def test_failed_v13_deferred_policy_migration_restores_v12_rows_atomically(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    migrations = database_module._load_migrations()
    legacy_migrations = tuple(item for item in migrations if item[0] <= 12)
    version, sql = next(item for item in migrations if item[0] == 13)
    monkeypatch.setattr(database_module, "_load_migrations", lambda: legacy_migrations)
    database = Database(tmp_path / "memory.db")
    database.initialize()
    with database.transaction() as connection:
    ...
    with database.connect() as connection:
        with pytest.raises(sqlite3.OperationalError, match="missing_v13_failure_table"):
            database_module._apply_migration(
                connection,
                version,
                f"{sql}\nselect * from missing_v13_failure_table;",
            )
    ...
    assert "parser_policy_sha256" not in columns
...
def test_readonly_schema_check_rejects_v12_without_migrating(
    tmp_path: Path,
) -> None:
    database = Database(tmp_path / "memory.db")
    database.initialize()
...
def test_initialize_creates_exact_contract_columns_indexes_and_triggers(
    ...
    assert tuple(row[0] for row in versions) == tuple(range(1, 14))
...
def test_codex_deferred_schema_rejects_private_or_malformed_locator_rows(
    ...
    invalid_rows = (
        ...
        invalid_row(35, parser_policy_sha256="B" * 64),
        invalid_row(36, parser_policy_sha256=sqlite3.Binary(b"b" * 64)),
        invalid_row(37, parser_policy_sha256=None),
        invalid_row(38, parser_policy_sha256="b" * 63),
...
def test_failed_migration_rolls_back_schema_and_version(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            *migrations,
            (14, "create table rolled_back(value text);\nthis is invalid sql;"),
        ),
    )
...
def test_migration_is_applied_only_once(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        database_module,
        "_load_migrations",
        lambda: (*migrations, (14, "create table migration_probe(value text);")),
    )
    ...
    with db.connect() as conn:
        assert (
            conn.execute("select count(*) from schema_migrations where version=14").fetchone()[0]
            == 1
        )
