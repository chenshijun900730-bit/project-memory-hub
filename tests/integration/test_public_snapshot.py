...
def _repository(tmp_path: Path) -> Path:
    ...
    _git(root, "add", "--all")
    _git(root, "commit", "-m", "public candidate")
    return root
...
def test_builds_deterministic_single_root_snapshot_and_reaudits_exact_source(
    ...
    assert _git(root, "show", "-s", "--format=%aI", BRANCH) == "2000-01-01T00:00:00Z"
    assert _git(root, "show", "-s", "--format=%cI", BRANCH) == "2000-01-01T00:00:00Z"
...
def test_rechecks_worktree_parent_without_following_a_symlink_race(
    ...
    def race_parent(repository: Path, tree: str) -> str:
        commit = cast(str, original(repository, tree))
