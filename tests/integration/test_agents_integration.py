...
def test_managed_block_requires_precise_project_memory_feedback(tmp_path: Path) -> None:
    launcher = _launcher(tmp_path)
    target = tmp_path / "AGENTS.md"
    ...
    required = (
        "Never report only",
        "project memory was unavailable",
        ...
        "newly queued for pending verification",
    ...
    assert all(fragment in document for fragment in required)
    assert "briefly disclose that project memory was unavailable" not in document
    assert "pending_verification` and `duplicate=false" in document
    assert "not trusted memory" in document
    assert "duplicate=true" in document
    assert "not a new write" in document
