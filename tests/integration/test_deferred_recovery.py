...
from project_memory_hub.services.deferred_recovery import (
    DeferredRecoveryService,
    _deferred_source_hash,
    _locator_anchor,
)
...
def test_codex_adapter_rejects_a_deferred_locator_from_an_old_parser_policy(
    tmp_path: Path,
    ...
    assert len(locator.parser_policy_sha256) == 64
    drifted = replace(locator, parser_policy_sha256="0" * 64)
    monkeypatch.setattr(
        codex_module,
        "_open_root",
        lambda _root: (_ for _ in ()).throw(AssertionError("source must not be opened")),
    )
...
def test_deferred_locator_rejects_a_checkpoint_without_parser_policy(
    tmp_path: Path,
) -> None:
    sessions = tmp_path / "sessions"
    sessions.mkdir()
    ...
    checkpoint = CodexAdapter(sessions, Redactor()).read_incremental(scope, None).next_checkpoint
    cursor = dict(checkpoint.cursor)
    cursor.pop("parser_policy_sha256")
    legacy_checkpoint = checkpoint.model_copy(update={"cursor": cursor})
...
def test_deferred_receipt_source_hash_binds_the_exact_parser_policy(
    tmp_path: Path,
) -> None:
    sessions = tmp_path / "sessions"
    sessions.mkdir()
    ...
    batch = CodexAdapter(sessions, Redactor()).read_incremental(scope, None)
    locator = CodexDeferredLocator.from_checkpoint(
        scope,
        "session-one:turn-one",
        batch.next_checkpoint,
    )
    record = CodexDeferredRecord(
        deferred_id="00000000-0000-4000-8000-000000000001",
        locator=locator,
        state="pending",
    )
    old_policy_record = replace(
        record,
        locator=replace(locator, parser_policy_sha256="0" * 64),
    )

    assert _deferred_source_hash(record) != _deferred_source_hash(old_policy_record)
    assert _locator_anchor(record) != _locator_anchor(old_policy_record)
