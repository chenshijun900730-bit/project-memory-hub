...
def _create_root_commit(repository: Path, tree: str) -> str:
    ...
    expected = (
        f"tree {tree}\n"
        f"author {FIXED_NAME} <{FIXED_EMAIL}> {FIXED_EPOCH}\n"
        f"committer {FIXED_NAME} <{FIXED_EMAIL}> {FIXED_EPOCH}\n"
        f"\n{FIXED_MESSAGE}"
    ).encode("ascii")
...
def prepare_public_snapshot(
    *,
    ...
    commit = _create_root_commit(root, tree)
    _validate_source_clean(root, source, tree)
