...
WHERE EXISTS (
    SELECT 1 FROM codex_deferred_records WHERE state = 'pending'
)
ON CONFLICT(name) DO UPDATE SET
    value_json = excluded.value_json,
    updated_at = excluded.updated_at;

DROP TABLE codex_deferred_records;
...
CREATE TABLE codex_deferred_records (
    ...
    parser_policy_sha256 TEXT NOT NULL CHECK (
        typeof(parser_policy_sha256) = 'text'
        AND length(parser_policy_sha256) = 64
        AND parser_policy_sha256 NOT GLOB '*[^0-9a-f]*'
    ),
