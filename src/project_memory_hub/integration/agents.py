...
def _managed_block(launcher: Path) -> bytes:
    ...
    lines = (
        ...
        "- Treat an MCP capture result with `status=pending_verification` and `duplicate=false` as a newly queued row pending model verification; the Codex JSONL adapter supplies trusted model provenance during reconcile. If `duplicate=true`, report the exact duplicate result instead of claiming that a new row was queued, because the matching row may already be verified or expired.",
        "- If context resolution fails, continue the user task and briefly disclose that project memory was unavailable.",
        "- If MCP reconcile fails or is unavailable, continue the user task and briefly disclose that project memory was unavailable.",
        "- If recall fails, continue the user task and briefly disclose that project memory was unavailable.",
        "- If MCP capture fails or is unavailable, keep the final capture marker pair for later trusted adapter recovery, briefly disclose that project memory was unavailable, and do not claim that a new row was queued unless the tool returned `status=pending_verification` with `duplicate=false`; do not withhold the user deliverable.",
        MANAGED_END,
        "",
    )
