...
class CaptureService:
    ...
    def capture_prepared_on_connection(
        ...
        *,
        ...
        if exact_pending_id is not None and prior_codex_receipt_proof is not None:
            # A prior Codex receipt can prove an ordinary idempotent replay, but it
            # must never relax the exact source-time binding used to promote an
            # untrusted pending row.
    ...
    def _existing_source_ref(
        ...
        # capture_prepared_on_connection excludes receipt proofs from exact-pending
        # recovery. Here a transaction-bound Codex receipt therefore proves only an
        # ordinary replay of the same already-bound source/content/project/model;
        # its event timestamp may move in either direction across parser versions.
        timestamp_is_proven_receipt_replay = (
        ...
        if (
            ...
            or not (timestamp_matches or timestamp_is_proven_receipt_replay)
        ):
            raise _IncompatibleSourceProvenance
