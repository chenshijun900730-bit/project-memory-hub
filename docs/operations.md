...
`reconcile`、`doctor` 和任何每日 reconcile 自动任务都不会调用来源探针。`doctor` 仍只检查既有运行时健康；来源探针只会由上面的显式 CLI 或本地 Sources 页面触发。来源探针本身不会触发 migration；0.2.1 当前运行 schema 为 v13，升级边界见下文 `0011_pending_capture_history.sql`、`0012_capture_correlation.sql` 与 `0013_codex_deferred_parser_policy.sql`。
...
Codex 不得只说“项目工程记忆仍未写入”或“Active Codex Context 无法识别”。每次 PMH
异常或状态反馈都必须明确包含以下五项：

1. **失败阶段**：只能标为 `codex-context`、`recall`、`capture_pending`、
   `pending verification` 或 `reconcile`。
2. **根因与证据**：只有证据能证明时才写“已确认”；否则必须写“尚未确认”。统一错误码只能作为证据，不能直接当作根因。
3. **实际记录状态**：说明本次是否尚未进入写入阶段、已排队、命中重复、已验证或已过期。
4. **代码提交影响**：独立说明 PMH 问题是否影响代码修改、测试结果或 Git 提交。
5. **安全补救**：列出 Codex 已执行的安全补救，并明确用户是否需要操作。

状态用语必须严格对应回执：

- context 解析失败且没有调用 capture：写“尚未进入写入阶段”，不得暗示内容违规或数据库损坏。
- `status=pending_verification` 且 `duplicate=false`：写“已新增到待验证队列，尚未成为可信记忆”。
- `duplicate=true`：写“命中已有记录，本次没有新写入”；仅凭该回执不得推断已有记录仍在 pending、已验证或已过期。
- 只有可信适配器或 reconcile 的精确证据才能写“已验证”或“已过期”；聚合计数不能证明某一条记录的状态。

例如，PMH 0.2.1 的 `codex-context` 若在完整解析前命中单记录 4 MiB 物理上限，而
`CODEX_THREAD_ID`、唯一会话文件、cwd 和 model 均已匹配，且受影响记录经独立诊断确认全部是
`image_generation_end` 或 `response_item/custom_tool_call_output`，应说明失败位于
`codex-context`、capture 尚未开始、根因由记录尺寸与类型证据确认，且这不等于记忆内容冲突、
权限不足或数据库损坏。本次 PMH 失败不影响已经验证的代码修改、测试或 Git 提交。Codex 的
安全补救是保留语义记录的 fail-closed 校验，只在更大的有界窗口内完整解析后跳过已知非语义
记录；永久修复部署后用户无需操作，部署前也可在同一 Git 项目的短任务中重新核验提交后补写。
...
## 历史升级：0.2.1 与 schema v11/v12/v13
...
`0013_codex_deferred_parser_policy.sql` 为每个 content-free Codex deferred locator 增加精确的解析策略 SHA-256，并把该指纹绑定到唯一键、重放校验、canonical anchor 和 receipt source hash。v12 旧行没有足够证据证明创建时的策略，迁移不会把当前指纹猜填给旧记录，而是在同一事务内失效旧 locator。只有存在旧 pending locator 时才写入专用的 `codex_reconcile_catchup_required`；下一次 reconcile 会因为旧 checkpoint 缺少当前策略指纹而从安全边界重新扫描，并按新策略重新生成需要的 locator。Codex scope 报告失败、运行异常或来源被禁用时该标记保持 due；只有没有 scope failure 的 Codex 补扫完成本轮提交时，才通过 compare-and-delete 清除它。迁移不读取或复制 capture 内容。
...
迁移范围应为 `1|13|13`，`quick_check` 必须仍为 `ok`，`foreign_key_check` 必须仍为空，而 `pending_captures` 只能包含 `pending`。v10 无法证明历史 verified 行对应的可信 `source_reference_id`，因此迁移后的该字段允许为空，不得猜测补写；新核验记录会保存真实引用。旧 expired 行以可验证的 `expires_at` 作为 `finalized_at`；旧 verified/rejected 行没有可信终结时间，因此使用严格有效的 `created_at` 作为保守排序代理。历史全局硬上限为 50000 条：迁移和后续写入都只保留按 `(finalized_at, pending_id)` 排序的最新元数据，active pending 不受这项历史淘汰影响。

如果 v13 升级前存在 pending deferred locator，完成 `init` 后执行 `memory-hub reconcile --force --format json`，确认 catch-up 已完成，再运行 `memory-hub doctor --format json`。不要手工删除 catch-up 标记或给旧行补写当前策略哈希。

如果 migration v11、v12 或 v13 失败，保持 writer 停止并按下文流程检查。回退必须恢复升级前已验证的 SQLite 备份；安装旧代码本身不能安全回退 schema。
